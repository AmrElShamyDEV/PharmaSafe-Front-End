...
export default function DDIChecker() {
  const savedMedicines = ["Panadol", "Amoxicillin", "Brufen", "Metformin"];
  ...
  const addManualRow = () => {
    setManualRows([...manualRows, { id: Date.now(), name: "" }]);
  };
  ...
  const runAnalysis = (medicines: string[]) => {
  ...
  const analyzeManual = () => runAnalysis(manualRows.map((r) => r.name));
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 text-slate-800">
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-6 pt-10 pb-6">
        ...
        <p className="mt-3 text-slate-600 text-lg">
          Check if medicines can be safely taken together.
        </p>
      </section>
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-100 p-6 md:p-8">
          ...
          <div className="flex flex-col md:flex-row gap-4 md:items-center md:justify-between mb-8">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button
                onClick={() =>
                  activeTab === "saved" ? analyzeSaved() : analyzeManual()
                }
                className="px-5 py-3 rounded-2xl bg-gradient-to-r from-blue-600 to-blue-500 text-white font-semibold hover:scale-105 transition"
              >
                {activeTab === "saved" ? "Check Interactions" : "Analyze DDI"}
              </button>
            </div>
          </div>
            ...
            <>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {filteredSaved.map((med) => {
                  const selected = selectedMeds.includes(med);
          ...
          {activeTab === "manual" && (
            <div className="space-y-4">
              {manualRows.map((row) => (
                ...
                >
                  <input
                    ...
                    value={row.name}
                    onChange={(e) => updateManualRow(row.id, e.target.value)}
                    className="px-4 py-3 rounded-xl border outline-none focus:ring-2 focus:ring-blue-500"
                  />

                  <button
                    onClick={() => deleteManualRow(row.id)}
                    className="px-4 rounded-xl bg-red-50 text-red-600 hover:bg-red-100 flex items-center gap-2"
                  >
              ...
              <button
                ...
                className="px-5 py-3 rounded-2xl bg-blue-50 text-blue-700 font-semibold flex items-center gap-2 hover:bg-blue-100 transition"
              >
          ...
          {/* Error */}
          {errorMsg && (
            <div className="mt-8 p-5 rounded-2xl bg-amber-50 border border-amber-200 text-amber-700 flex items-center gap-3">
          ...
          {loading && (
            <div className="mt-8 p-6 rounded-2xl bg-blue-50 border border-blue-100 flex items-center gap-3">
              ...
              <span className="font-medium">
                AI is analyzing drug interactions...
              </span>
            </div>
          )}
            ...
            <>
              <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4 mt-8">
                {[
                  ...
                  ["Safe Pairs", safeCount, "bg-green-50 text-green-700"],
                  ["High Risk Alerts", severeCount, "bg-red-50 text-red-700"],
                ].map(([title, value, style]) => (
                  <div key={title} className={`rounded-2xl p-5 ${style}`}>
              ...
              <div className="mt-8 space-y-4">
                {results.map((item, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={`p-5 rounded-2xl border hover:-translate-y-1 hover:shadow-md transition ${item.color}`}
                    >
                      <div className="flex gap-3">
                        ...
                        <div>
                          ...
                          <p className="font-semibold mt-1">{item.pair}</p>
