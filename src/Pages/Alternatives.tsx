...
export default function Alternatives() {
  const prescriptionMeds = ["Panadol", "Amoxicillin", "Brufen", "Metformin"];

  const historyChips = ["Panadol", "Augmentin", "Brufen", "Metformin"];
  ...
  const allResults = [
    {
      ...
      price: 4,
      note: "Same active ingredient and commonly available in pharmacies.",
    },
    {
      ...
      price: 8,
      note: "Alternative antibiotic with similar therapeutic effect.",
    },
    {
      original: "Brufen",
      ...
      availability: "Available",
      price: 3,
      note: "Same pain relief effect with lower average price.",
    },
    {
      original: "Metformin",
      ...
      price: 7,
      note: "May be preferred depending on tolerance and profile.",
    },
  ];
  ...
  const runSearch = (medicine: string) => {
    if (!medicine) return;
    ...
    setTimeout(() => {
      ...
      const filtered = allResults.filter((r) =>
        r.original.toLowerCase().includes(medicine.toLowerCase()),
      );
  ...
  const filteredResults = [...results].filter((item) => {
    ...
    if (filter === "Cheapest") return item.price <= 4;
    if (filter === "Same Ingredient") return item.type === "Same Ingredient";
    if (filter === "Similar Effect") return item.type === "Similar Effect";
  ...
  const cheapest =
    results.length > 0 ? `$${Math.min(...results.map((r) => r.price))}` : "-";
  ...
  const badgeType = (type: string) => {
    if (type === "Same Ingredient") return "bg-blue-100 text-blue-700";
    if (type === "Similar Effect") return "bg-emerald-100 text-emerald-700";
    if (type === "Lower Price") return "bg-yellow-100 text-yellow-700";
    return "bg-purple-100 text-purple-700";
  };

  const badgeAvailability = (value: string) => {
    if (value === "Available") return "bg-green-100 text-green-700";
    if (value === "Limited") return "bg-yellow-100 text-yellow-700";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 text-slate-800">
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-6 pt-10 pb-6">
        ...
        <p className="mt-3 text-slate-600 text-lg">
          Find safer, cheaper, or available alternatives instantly.
        </p>
      </section>
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-100 p-6 md:p-8">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {prescriptionMeds.map((med) => (
                  <button
                    ...
                    className={`p-5 rounded-2xl border text-left transition hover:-translate-y-1 hover:shadow-md ${
                      selectedMed === med
                  ...
                  >
                    ...
                    <div className="font-semibold">{med}</div>
                  </button>
                ))}
              </div>
          ...
          {tab === "search" && (
            <>
              ...
              <div className="mt-5 flex gap-3 flex-wrap">
                ...
                <button
                  onClick={clearAll}
                  className="px-5 py-3 rounded-2xl bg-slate-100 hover:bg-slate-200 flex gap-2 items-center"
                >
                  <X className="w-4 h-4" />
                  Clear
                </button>
          ...
          {loading && (
            <div className="mt-8 p-6 rounded-2xl bg-blue-50 border border-blue-100 flex items-center gap-3">
              ...
              AI is searching best alternatives...
            </div>
          )}
          ...
          {!loading && results.length === 0 && (
            <div className="mt-8 p-6 rounded-2xl bg-slate-50 border border-slate-200 text-center">
              ...
              <h3 className="font-bold text-lg">Ready to Search</h3>
              <p className="text-slate-500 mt-1">
                Select or search a medicine to discover alternatives.
              </p>
            </div>
          )}
          ...
          {results.length > 0 && !loading && (
            <>
              ...
              <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4 mt-8">
                {[
                  ["Alternatives Found", results.length],
                  ["Cheapest Option", cheapest],
                  ["Same Ingredient", sameMatches],
                  ...
                  >
                    <div className="flex justify-between gap-4 flex-wrap">
                      <div>
                        ...
                        <h3 className="font-bold text-lg">{item.original}</h3>
                      </div>
                    ...
                    <div className="mt-4 flex gap-2 flex-wrap">
                      ...
                      <span className="px-3 py-2 rounded-full text-xs font-semibold bg-slate-100 flex items-center gap-1">
              ...
              <div className="mt-6">
                <button
                  onClick={() => runSearch(selectedMed || search)}
                  className="px-5 py-3 rounded-2xl bg-slate-100 hover:bg-slate-200 font-semibold flex gap-2 items-center"
                >
